...
using System.Linq;
using WorldCupTeams.Domains;
using WorldCupTeams.DTOS;
using WorldCupTeams.Services;

namespace WorldCupTeams.Service
{
    public class WorldCupService : IWorldCupService
    {
            ...
            foreach (var item in teams)
            {
                teamsDto.Add(
                    new TeamDTO()
                    {
                        ...
                        Titles = item.Titles,
                        Name = item.Name,
                        Players = item.Players.Select(x => new PlayerDTO() { Key = x.Key, Name = x.Name, Team = x.Team, Age = x.Age }).ToArray()
                    });
            }
        ...
        public TeamDTO FindTeamBy(int key)
        {
            ...
            return new TeamDTO()
            {
                ...
                Titles = team.Titles,
                Name = team.Name,
                Players = team.Players.Select(x => new PlayerDTO() { Key = x.Key, Name = x.Name, Team = x.Team, Age = x.Age }).ToArray()
            };
        }
    }
}
